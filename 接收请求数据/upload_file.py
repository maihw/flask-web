...
UPLOAD_FOLDER = '/home/shiyanlou/flask-web'
ALLOWED_EXTENSIONS = set(['txt', 'pdf', 'png', 'jpg', 'gif'])
#允许上传的文件类型
app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER

def allowed_file(filename):#验证上传的文件名是否符合要求，
#文件名必须带点并且符合允许上传的文件类型要求
#两者都满足则返回True
...
def upload_file():
    if request.method == 'POST':#如果是POST请求方式
        ...
        if file and allowed_file(file.filename):#如果文件存在并且符合要求则为true
...
file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))#保存文件
    ...
    #使用get方式请求页面时或是上传文件失败时返回上传文件的表单页面
    return '''
    <!doctype html>
